 ...
 * @return array
 */
function energico_related_posts() {

	if ( ! is_singular( 'post' ) ) {
		return;
	}

	$visible = get_theme_mod( 'related_posts_visible', energico_theme()->customizer->get_default( 'related_posts_visible' ) );
	...
	$post_terms = '';
	$post_number = get_theme_mod( 'related_posts_count', energico_theme()->customizer->get_default( 'related_posts_count' ) );
	...
	$utility = energico_utility()->utility;
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = get_theme_mod( $setting_value, energico_theme()->customizer->get_default( $setting_value ) );
	}

	$settings['title_visible'] = $settings[ 'title_length' ] ? get_theme_mod( 'related_posts_title', energico_theme()->customizer->get_default( 'related_posts_title' ) ) : false;
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = $utility->media->get_image( array(
				...
				'size'			=> 'energico-thumb-m',
				'mobile_size'	=> 'energico-thumb-m',
			) );
