 ...
 */
add_filter( 'template_include', array( 'Energico_Wrapping', 'wrap' ), 99 );
add_filter( 'energico_wrap_base', 'energico_wrap_base_cpts' );
 ...
 */
function energico_template_path() {
	return Energico_Wrapping::$main_template;
}
 ...
 */
function energico_template_base() {
	return Energico_Wrapping::$base;
}

if ( ! class_exists( 'Energico_Wrapping' ) ) {
	...
	class Energico_Wrapping {
		...
		/**
		 * Initialize new instance of Energico_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'energico_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in Energico_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function energico_wrap_base_cpts( $templates ) {
