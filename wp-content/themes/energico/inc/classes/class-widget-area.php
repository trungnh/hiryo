...
if ( ! class_exists( 'Energico_Widget_Area' ) ) {

	class Energico_Widget_Area {
		...
		function __construct() {
			...
			add_action( 'energico_render_widget_area', array( $this, 'render' ) );
		}
		...
		public function render( $area_id ) {
			...
			$area_id        = apply_filters( 'energico_rendering_current_widget_area', $area_id );
			...
			$classes = apply_filters( 'energico_widget_area_classes', $classes, $area_id );
	...
	function energico_widget_area() {
		return Energico_Widget_Area::get_instance();
	}

	energico_widget_area();
}
