...
if ( ! class_exists( 'Energico_Custom_Posts_Widget' ) ) {

	class Energico_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'energico' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'energico' );
			$this->widget_id          = apply_filters( 'energico_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'energico_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = energico_utility()->utility;
			$this->settings           = array(
				'title'          => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'energico' ),
				...
				'terms_type'     => array(
					...
					'options' => array(
						'category_name' => array(
							'label' => esc_html__( 'Category', 'energico' ),
							'slave' => 'terms_type_post_category',
						),
						'tag'           => array(
							'label' => esc_html__( 'Tag', 'energico' ),
							'slave' => 'terms_type_post_tag',
						),
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'energico' ),
							'slave' => 'terms_type_post_format',
						),
					),
					'label'   => esc_html__( 'Choose taxonomy type', 'energico' ),
				),
				'category_name'  => array(
					'type'             => 'select',
					'size'             => 1,
					...
					'options'          => false,
					'label'            => esc_html__( 'Select category', 'energico' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select category', 'energico' ),
				...
				'tag'            => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select tags', 'energico' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select tags', 'energico' ),
				...
				'post_format'    => array(
					...
					'options_callback' => array(
						$this->utility->satellite,
						'get_terms_array',
						array( 'post_format', 'slug' )
					),
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'energico' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'energico' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'energico' ),
				),
				'post_offset'    => array(
					'type'       => 'stepper',
					'value'      => '0',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'energico' ),
				),
				'title_length'   => array(
					'type'       => 'stepper',
					'value'      => '10',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'energico' ),
				),
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'energico' ),
				),
				'meta_data'      => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						'date'          => esc_html__( 'Date', 'energico' ),
						'author'        => esc_html__( 'Author', 'energico' ),
						'comment_count' => esc_html__( 'Comment count', 'energico' ),
						'category'      => esc_html__( 'Category', 'energico' ),
						'post_tag'      => esc_html__( 'Tag', 'energico' ),
						'more_button'   => esc_html__( 'More Button', 'energico' ),
					),
					'label'   => esc_html__( 'Display post meta data', 'energico' ),
				),
				'button_text'    => array(
					'type'  => 'text',
					'value' => 'Read More',
					'label' => esc_html__( 'Post read more button label', 'energico' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$grid_class_array         = array(
				'default'             => 'col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12',
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'        => 'energico-thumb-s',
						'mobile_size' => 'energico-thumb-s',
	...
	add_action( 'widgets_init', 'energico_register_custom_posts_widget' );
	function energico_register_custom_posts_widget() {
		register_widget( 'Energico_Custom_Posts_Widget' );
	}
}
